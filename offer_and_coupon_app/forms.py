...
class ProductOfferForm(forms.ModelForm):
    ...
    def clean_discount_value(self):
        discount = self.cleaned_data['discount_value']
        if discount <= 0 or discount > 100:
            raise ValidationError("Discount value must be between 0.01 and 100.")
        return discount

    def clean_min_purchase_amount(self):
        amount = self.cleaned_data['min_purchase_amount']
        if amount < 0:
...
class CategoryOfferForm(forms.ModelForm):
    ...
    class Meta:
        model = CategoryOffer
        fields = ['name', 'description', 'discount_value', 'min_purchase_amount', 'max_discount_amount', 'valid_from', 'valid_to', 'is_active', 'categories']
        widgets = {
            'name': forms.TextInput(attrs={'class': 'form-control'}),
            'description': forms.Textarea(attrs={'class': 'form-control'}),
            'discount_value': forms.NumberInput(attrs={'class': 'form-control', 'step': '0.01'}),
